...
class SimulatedAnnealingBase(SkoBase):
    ...
    def __init__(self, func,lb,ub, x0=None, T_max=100, T_min=1e-7, L=300, max_stay_counter=300,max_iter=500, **kwargs):
        assert T_max > T_min > 0, 'T_max > T_min > 0'

        self.func = func
        self.lb=lb if isinstance(lb,np.ndarray) else np.array(lb)
        self.ub=ub if isinstance(ub,np.ndarray) else np.array(ub)
        ...
        self.n_dims = len(x0)
        if x0 is None: # initial solution
            self.x=np.random.random(size=self.lb.shape) * (self.ub - self.lb) + self.lb
        else:
            self.x = np.array(x0)
        # self.best_x = np.array(x0)  # initial solution
        self.best_x = self.x  # initial solution
    ...
    def get_new_x(self, x):
        ...
        x_new[np.where(x_new<self.lb)]=self.lb[np.where(x_new<self.lb)]
        x_new[np.where(x_new > self.ub)] = self.ub[np.where(x_new > self.ub)]
        return x_new
    ...
    def run(self):
        ...
        while it<self.max_iter:
            ...
            if self.T < self.T_min:
                ...
                print(stop_code)
                break
            if stay_counter > self.max_stay_counter and False:
                stop_code = 'Stay unchanged in the last {stay_counter} iterations'.format(stay_counter=stay_counter)
                print(stop_code)
                break
            print('Iteration # %d #, T=%f' % (self.iter_cycle,self.T), 'best_x is ' + str(self.best_x), 'best_y is ' + str(self.best_y))
...
class SAFast(SimulatedAnnealingBase):
    ...
    def __init__(self, func,lb,ub, x0, T_max=100, T_min=1e-7, L=300, max_stay_counter=150, **kwargs):
        super().__init__(func,lb,ub, x0, T_max, T_min, L, max_stay_counter, **kwargs)
